package edu.cqu.algorithms.Treebb.LFU;
...
public abstract class RetentionPlusFrameLFU extends RetentionFrameLFU {

    private ArrayList<OptHistory> optHistories;
    private Map<Integer,Integer> proportion;

    public RetentionPlusFrameLFU(int id, int[] domain, int[] neighbours, Map<Integer, int[][]> constraintCosts, Map<Integer, int[]> neighbourDomains, SyncMailer mailer) {
...
//        System.out.println("proportion ：" + proportion);
    ...
    private void showOptHistory() {
        ...
        for (OptHistory optHistory : optHistories) {
            System.out.println("count : " +  optHistory.hitCount);
...
//                        return optHistory.hitCount - t1.hitCount;
                        return t1.hitCount - optHistory.hitCount;
                    }
                });
//                showOptHistory();
                int removeID = -1;
                ...
                for (int i = optHistories.size() - 1; i >= 0; i--) {
                    if (optHistories.get(i).childId == child) {
                        maxCount = optHistories.get(i).hitCount;
                        break;
                    }
                }
                for (int i = 0; i < optHistories.size(); i++) {
                    if (optHistories.get(i).childId == child && optHistories.get(i).hitCount == maxCount) {
                        removeID = i;
                        break;
                    }
                }
