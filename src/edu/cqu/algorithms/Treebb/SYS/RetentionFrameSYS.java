...
public abstract class RetentionFrameSYS extends DFSSyncAgent {

    private static final int MSG_BRANCH_PSEUDO_PARENTS = 0XFFFF4;
    private static final int MSG_HEIGHT = 0XFFFF5;
    protected static final int INFINITE = Integer.MAX_VALUE;
    private static final int MSG_ECA = 0XFFFF6;
    ...
    protected Map<Integer, Integer> ancHeight;

    public RetentionFrameSYS(int id, int[] domain, int[] neighbours, Map<Integer, int[][]> constraintCosts, Map<Integer, int[]> neighbourDomains, SyncMailer mailer) {
        ...
        switch (message.getType()) {
            case MSG_BRANCH_PSEUDO_PARENTS: {
                ...
                if (branchPseudoParents.size() == children.size()) {
                    System.out.println("id : " + id + " branchPseudoParents : " + branchPseudoParents + "height : " + height);
                    if (!isRootAgent()) {
...
//                int sender = message.getIdSender();
                Map<Integer, Integer> temp = (Map<Integer, Integer>) message.getValue();
//                for (int k : temp.keySet()) {
//                    ancHeight.put(k, temp.get(k));
//                }
                ancHeight.putAll(temp);
//                System.out.println("id : " + id + "anc: " + ancHeight);
                temp.put(id, height);
                for (int child : children) {
                    sendMessage(new Message(id, child, MSG_HEIGHT, temp));
                }
...
//        if (isRootAgent()) {
//            Map<Integer, Integer> sendValue = new HashMap<>();
//            sendValue.put(id, height);
////            System.out.println("===" + sendValue);
//            for (int child : children) {
//                sendMessage(new Message(id, child, MSG_HEIGHT, sendValue));
//            }
//        }
        if (isLeafAgent()) {
